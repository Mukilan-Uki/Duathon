  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-10 print:p-0">
      <section className="rounded-2xl border border-slate-300 bg-white p-8 print:border-0">
        ...
        <p className="my-8 text-center text-3xl font-bold">
          {formatMinorUnits(receipt.amountMinor, receipt.currency)}
        </p>
        <dl className="space-y-4">
          {[
            ['Reference', receipt.reference],
            ['Transfer reference', receipt.transferReference],
            ['Date', new Date(receipt.date).toLocaleString()],
            ['Direction', receipt.direction],
            ['Counterparty account', receipt.counterpartyAccountNumber],
        ...
        <p className="mt-8 text-center text-xs text-slate-500">
          This computer-generated receipt does not require a signature.
        </p>
      </section>
      <button
        className="mx-auto mt-6 block rounded-lg bg-bank-700 px-5 py-2.5 font-semibold text-white print:hidden"
